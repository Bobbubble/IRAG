...
def extract_nodes_edges(desc):
    ...
    edge_set = set(edge_lines)

    node_data = [line.split(",", 1) for line in node_set]
    edge_data = [line.split(",", 2) for line in edge_set]

    node_df = pd.DataFrame(node_data, columns=["node_id", "node_attr"])
    edge_df = pd.DataFrame(edge_data, columns=["src", "edge_attr", "dst"])
...
class WebQSPDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        id_to_attr = dict(zip(nodes['node_id'], nodes['node_attr']))
        ...
        # 限制正样本的数量
        max_positive = 10
        src_ids, dst_ids = torch.tril(torch.ones(num_nodes, num_nodes, dtype=torch.bool), diagonal=-1).nonzero(as_tuple=True)
        candidate_edge_index = torch.stack([src_ids.flatten(), dst_ids.flatten()], dim=0)
        all_edges = list((u.item(), v.item()) for u, v in graph.edge_index.t())

        # 随机采样 max_positive 条正样本边
        if len(all_edges) > max_positive:
            existing_edges_list = random.sample(all_edges, max_positive)
        else:
            existing_edges_list = all_edges

        existing_edges_set = set(existing_edges_list)
        existing_edges_attr = {(u.item(), v.item()): edge_attr[i] for i, (u, v) in enumerate(graph.edge_index.t()) if
                               (u.item(), v.item()) in existing_edges_set}
        ...
        for (u, v) in negative_edges_set:
            ...
            negative_edge_attr.append(torch.zeros(edge_attr.size(1)))
            negative_edge_text.append('')
        ...
        candidate_edge_index = torch.tensor(all_edge_index, dtype=torch.long).T
        candidate_edge_attr = torch.stack(all_edge_attr, dim=0)
        edge_label = torch.tensor(all_labels, dtype=torch.long)
